...
def add_sensor_to_mongodb(mongodb_client: MongoDBClient, db_sensor: schemas.SensorCreate, id):
    mongo_projection = schemas.SensorMongoProjection(id=id, name=db_sensor.name, location={'type': 'Point', 'coordinates': [db_sensor.longitude, db_sensor.latitude]}, type=db_sensor.type, mac_address=db_sensor.mac_address, description=db_sensor.description, serie_number=db_sensor.serie_number, firmware_version=db_sensor.firmware_version, model=db_sensor.model, manufacturer=db_sensor.manufacturer)
